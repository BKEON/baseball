...
const QUESTIONS = [
  {
    ...
    options: [
      ...
      { value: "가족피크닉", label: "☀️ 날씨 좋은데 돗자리 펴고 놀면 좋겠다~" },
  ...
  },
  {
    ...
    options: [
      ...
      { value: "먹방흡입", label: "🍺 한 손엔 떡볶이, 한 손엔 맥주 끊임없이 흡입" },
...
export default function TestPage() {
  ...
  useEffect(() => {
    if (!started) { trackTestStart(); setStarted(true); }
  }, [started]);
  ...
  const handleNext = async () => {
    ...
    if (currentQ < QUESTIONS.length - 1) {
      setDirection(1); setCurrentQ((q) => q + 1); setSelected(null);
    } else {
  ...
  const handlePrev = () => {
    if (currentQ > 0) {
      setDirection(-1); setCurrentQ((q) => q - 1);
    ...
    <main className="min-h-screen bg-hw-dark diamond-pattern flex flex-col">
      ...
      <div className="flex items-center justify-between px-5 pt-5 pb-3">
        <button onClick={handlePrev} disabled={currentQ === 0}
          className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-white/40 disabled:opacity-20 hover:text-white transition-all">←</button>
        <div className="font-mono text-sm"><span className="text-hw-orange">{currentQ + 1}</span><span className="text-white/30"> / {QUESTIONS.length}</span></div>
        <div className="w-10" />
      </div>

      <div className="px-5 mb-8">
        <div className="h-1 bg-white/10 rounded-full overflow-hidden">
          <motion.div className="h-full bg-hw-orange rounded-full progress-glow"
      ...
      <div className="flex-1 px-5 flex flex-col">
        <AnimatePresence mode="wait">
          <motion.div key={currentQ}
            initial={{ opacity: 0, x: direction * 50 }} animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -direction * 50 }} transition={{ duration: 0.3 }}
            className="flex-1 flex flex-col">
            <div className="text-7xl font-black text-white/5 leading-none mb-2">{question.number}</div>
            <h2 className="text-xl font-bold text-white leading-relaxed mb-8 whitespace-pre-line break-keep">{question.text}</h2>
            <div className="flex flex-col gap-3 flex-1">
              {question.options.map((opt, i) => (
                <motion.button key={opt.value}
                  ...
                  <span className="text-white/90 break-keep">{opt.label}</span>
                  {selected === opt.value && (
      ...
      <div className="px-5 pb-8 pt-4 safe-bottom">
        <motion.button onClick={handleNext} disabled={!selected} whileTap={{ scale: 0.97 }}
          className={`btn-primary w-full py-5 rounded-2xl text-lg tracking-widest font-bold transition-all ${!selected ? "opacity-30 cursor-not-allowed" : ""}`}>
